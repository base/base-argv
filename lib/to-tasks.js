...
module.exports = function toTasks(args, app, prop) {
  prop = prop ? utils.pluralize(prop) : 'apps';
  if (!app) app = {};

  args = utils.arrayify(args);
  var tasks = reduceTasks(args);
  ...
  while (++i < len) {
    ...
    // if no val is defined, see if it's a task
